...
insert into AUDITORIUM_TYPE(AUDITORIUM_TYPE,AUDITORIUM_TYPENAME) values('ЛК_17','Лекционка');
...
-- ограничения целостности выполняется до срабатывания AFTER-триггера.
 	  update TEACHER set GENDER = 'й' where TEACHER='Ив'
	  select * from TR_AUDIT
 --6
...
--а также систем-ную процедуру SP_SETTRIGGERORDERS. 
   insert into FACULTY(FACULTY) values ('Карленок')
go   
...
exec  SP_SETTRIGGERORDER @triggername = 'AUD_AFTER_DEL2', 
 ...
 delete FACULTY where FACULTY='Карленок'
