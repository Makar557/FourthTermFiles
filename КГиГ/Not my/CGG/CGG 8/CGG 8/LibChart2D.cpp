...
CMatrix SpaceToWindow(CRectD& RS, CRect& RW)//Пространиство в окно
{
	CMatrix M(3, 3);
	CSize sz = RW.Size();	
	int dwx = sz.cx;	     
...
void CPlot2D::SetWindowRect(CRect& RWX)//задаёт прямоугольник для рисования
{
	RW.SetRect(RWX.left, RWX.top, RWX.right, RWX.bottom);	
	K = SpaceToWindow(RS, RW);		
}
...
//-----------------------------------------------------------------
void CPlot2D::Draw(CDC& dc, int Ind1, int Ind2)//рисование
{
	double xs, ys; 
	int xw, yw; 
...
void CPlot2D::Draw1(CDC& dc, int Ind1, int Ind2)//рисование
{
