...
void CPyramid::ColorDraw(CDC& dc,CMatrix& PView,CRect& RW,COLORREF Color) 
{
	...
	CMatrix R1(3),R2(3),VN(3);
	double sm;//коэфициент преломления
	for(int i=0;i<4;i++)	
	{
		int k;
		if(i==3) k=0;
		...
		//передаем в вектор
		CMatrix V1=R2-R1; //третья точка между двумя в основании			    
		CMatrix V2=VE-R1; //третья точка между точкой в основании и вершиной		      
		VN=VectorMult(V2,V1);		  	

		sm=CosV1V2(VN,ViewCart); //косинус между сторонойв пирамиде и прямой до камеры	
	...
	//то же что и в цикле, но задает начальное значение, если sm=0
	VN=VectorMult(R1,R2);	
