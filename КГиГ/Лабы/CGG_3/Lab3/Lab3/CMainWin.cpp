...
//Режимы отображения MM_ISOTROPIC (изотропный) и MM_ANISOTROPIC (анизотропный) 
//допускают изменение направления осей X и Y, а также изменение масштаба осей координат.
...
void CMainWin::PaintF2(CDC& dc)
{
	...
	wind.top = wind.left = 100;
	view.top = view.left = 200;

	SetMyMode(dc, view, wind);
	...
	paint2D.SetPenAxis(penAxis);
	paint2D.SetPenLine(penLine);
	paint2D.SetWindowRect(CRect(100, 50, 400, 250));
	int count = 10 / 0.25;
	CMatrix X(count), Y(count);
	int j = 0;
...
void CMainWin::PaintF3(CDC& dc)
{
	...
	CMyPen penLine, penAxis;
	penAxis.Set(PS_SOLID, 2, RGB(0, 0, 255));
	penLine.Set(PS_DASHDOT, 3, RGB(0, 255, 0));
	paint2D.SetPenAxis(penAxis);
	paint2D.SetPenLine(penLine);
	paint2D.SetWindowRect(CRect(300, 50, 700, 400));
...
void CMainWin::PaintF4(CDC& dc)
{
	...
	CMyPen penLine, penAxis;
	penAxis.Set(PS_SOLID, 2, RGB(0, 0, 255));
	penLine.Set(PS_SOLID, 2, RGB(0, 255, 0));
	paint2D.SetPenAxis(penAxis);
	paint2D.SetPenLine(penLine);
	paint2D.SetWindowRect(CRect(300, 50, 700, 400));
	int count = 20 / 0.25;
...
void CMainWin::PaintF1234(CDC& dc)
{
	...
	penLine1.Set(PS_SOLID, 1, RGB(0, 255, 0));
	...
	penAxis3.Set(PS_SOLID, 2, RGB(0, 0, 255));
	penLine3.Set(PS_DASHDOT, 3, RGB(0, 255, 0));
	...
	penLine4.Set(PS_SOLID, 2, RGB(0, 255, 0));
