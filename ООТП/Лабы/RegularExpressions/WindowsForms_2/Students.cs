...
namespace WindowsForms_2
{
    public partial class Students : Form
    {
        ...
        public Students()
        {
            ...
            using (FileStream fstream = new FileStream(@"Students.xml", FileMode.Open))
            {
        ...
        private void SaveForm_Button_Click(object sender, EventArgs e)
        {
            ...
            if(IfWork_CheckBox.Checked)
            {
                ...
                tempStudent = new Student(LastName_MaskedTextBox.Text, FirstName_MaskedTextBox.Text,
                    TrirdName_MaskedTextBox.Text, Convert.ToInt32(Age_MaskedTextBox.Text), DateOfBirth_DateTimePicker.Value,
                    AverageMark_TrackBar.Value, course,  sex, specialty, new Adress(City_MaskedTextBox.Text,
                ...
                var context = new ValidationContext(tempStudent);
                if (!Validator.TryValidateObject(tempStudent, context, results, true))
                {
            ...
            XmlSerializer xmlFormat = new XmlSerializer(studentsList.GetType());
            using (FileStream fstream = new FileStream(@"..\Students.xml", FileMode.OpenOrCreate))
            {
        ...
        {
            MessageBox.Show("Создатель: Кубик Е.В.");
        }
